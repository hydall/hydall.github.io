...
function shuffleTeams(event) {
  ...
  if (players.length < 3) {
    showTooltip("•    Too few players - at least three players required");
    isExecuting = false;
    return;
  } else if (players.length > 10) {
    showTooltip("•    Too many players - can't shuffle more than 10 players");
    isExecuting = false;
    return;
  }
...
function colorizeHash(str, team) {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (team)
      result += ` <span class="blueplayer">#${hashtag}</span>${restOfText}`;
